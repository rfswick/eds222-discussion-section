...
# 2
# The lm(water_temp ~ latitude, pie_crab)) model fits better, which is what you would expect based on the graph of residuals.
```
...
# The intercept tells you the average mpg in 1999

# The coefficient estimate on the 2008 vintage indicator variable tells you the difference between the average mpg in 1999 and the average mpg in 2008. These two categorical variables are very similarly distributed.
```

4.  Does your model suggest anything about whether fuel efficiency has evolved over time? Why or why not?

**Answer: It suggests that is has not changed much over time.**
...
```{r}
# will only ever have one tilda (~)
summary(lm(hwy ~ year_fct + displ, mpg))
```
