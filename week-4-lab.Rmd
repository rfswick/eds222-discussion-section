...
2.  What does the intercept tell you, in words?
    ...
    The intercept is the mpg (35.7922) of an infinitesimally small engine in 1999.

3.  What does the coefficient on `displ` tell you, in words?

    The coefficient of `displ` is the slope of the linear regression between mpg and engine size in 1999.

4.  What does the coefficient estimate on the 2008 vintage indicator variable (`year2008`) tell you, in words?

    The coefficient of `year2008` is the difference between the intercept of the linear regression between mpg and engine size in 1999 and the linear regression between mpg and engine size in 2008.

5.  What does the coefficient on `displ:year2008` tell you, in words?[^week-4-lab-1]

    The coefficient of `displ:year2008` is the difference between the slope of the linear regression in 1999 and 2008.
...
```{r}
hwy_lm <- lm(hwy ~ displ + year + displ:year, data = mpg)
augment(hwy_lm)
...
ggplot(mpg, aes(displ, hwy, color = year)) +
  geom_point() +
  geom_abline(slope = coef(hwy_lm)["displ"],
              intercept = coef(hwy_lm)["(Intercept)"],
              color = "coral") + # x2 = 0 OR year = 1999
  geom_abline(slope = coef(hwy_lm)["displ"] + coef(hwy_lm)["displ:year2008"],
              intercept = coef(hwy_lm)["(Intercept)"] + coef(hwy_lm)["year2008"],
              color = "blue2")
