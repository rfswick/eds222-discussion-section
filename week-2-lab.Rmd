...
```{r}
cor(pie_crab$latitude, pie_crab$size)
cor(pie_crab$latitude, pie_crab$water_temp)
```
...
```{r}
# 1
lm(size ~ latitude, pie_crab)
summary(lm(size ~ latitude, pie_crab))

# 2 $\beta_0$ = -3.6244 (intercept), and $\beta_1$ = 0.4851 (slope)

# 3 The intercept $\beta_0$ is not directly interpretable because it is negative and size can not be negative
```
...
```{r}
pie_crab2 <- pie_crab
crab_lm <- lm(size ~ latitude, pie_crab)

# predict
# resid

pie_crab2 <- pie_crab2 %>% 
  mutate(predicted_size = predict(crab_lm),
         size_residual = resid(crab_lm))
```

2.  Make a histogram of your residuals. Do they look approximately normally distributed?

```{r}
ggplot(data = pie_crab2,
       aes(x = size_residual)) +
  geom_histogram()
```
